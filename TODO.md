...
* git ignore the compiled/minified js files, b/c those are generated by grunt and don't need to be tracked in the dev version, just shipped in the packaged version
	* won't be obvious to all contributors, though, so they might think it's broken if they clone repo and don't run `grunt`
	* maybe need a readme.md to describe git repo, and svn:ignore it.
